...
-- 3. Update proxies table - ADD THE MISSING TYPE COLUMN FIRST
ALTER TABLE proxies 
ADD COLUMN IF NOT EXISTS type TEXT DEFAULT 'sticky',
ADD COLUMN IF NOT EXISTS health TEXT DEFAULT 'unknown',
ADD COLUMN IF NOT EXISTS meta JSONB DEFAULT '{}'::jsonb,
ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW();

-- Add CHECK constraints AFTER adding the columns
...
-- 6. Update tasks table - ADD THE MISSING TYPE COLUMN OR RENAME task_type
-- Option 1: Add 'type' column and populate from task_type
ALTER TABLE tasks 
ADD COLUMN IF NOT EXISTS type TEXT,
ADD COLUMN IF NOT EXISTS meta JSONB DEFAULT '{}'::jsonb,
ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW();

-- Populate the new 'type' column from 'task_type' if it's empty
